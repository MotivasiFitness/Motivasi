...
export async function testProgramGeneration(
  ...
  try {
    const response = await fetch('/_functions/generateProgram', {
      method: 'POST',
    ...
    if (!response.ok) {
      ...
      return {
        endpoint: '/_functions/generateProgram',
        status: 'error',
...
export async function runApiDiagnostics(): Promise<ApiTestResult[]> {
  ...
  const healthResult = await testApiEndpoint('/_functions/health', 'GET');
...
export async function interactiveApiTest(): Promise<void> {
  ...
  const generateEndpointExists = results.some(
    r => r.endpoint === '/_functions/generateProgram' && r.status === 'success'
  );
